...
function StickyHeaderTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await axios.get(
        "http://127.0.0.1:5000/company/getClients",
        config
      );
  ...
  const onAddSubmit = async (e: any) => {
    try {
      console.log("first");
      e.preventDefault();
      const { _id, email, number, address, location, name, password } =
        addSelectedUser;
      const userData = {
        _id,
        email,
      ...
      const res = await axios.post(
        "http://127.0.0.1:5000/company/addClient",
        userData,
        config
      );
      console.log(res);
  ...
  const onSubmit = async (e: any) => {
    try {
      ...
      const res = await axios.put(
        "http://127.0.0.1:5000/company/editClient",
        userData,
        config
      );
      console.log(res);
  ...
  const deleteUserclick = async (e: any) => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/company/deleteUser",
        userData
      );
  ...
  return (
    <>
      ...
      {data && (
        <>
                  <Button onClick={() => setShow(true)} style={{marginBottom:'10px'}}>ADD Client</Button>
